...
async fn main() {
    ...
    // Build the HTTP request to fetch the DMs  ，  构造请求头。
    ...
    let response = request_sender.send_request(request).await.unwrap();   //接收到response
    ...
    // Upgrade the prover to an HTTP prover, and start notarization.   将证明程序升级为HTTP证明程序，然后开始公证。
    ...
    // Dump the notarized session to a file  将公证会话数据保存到文件的代码块。
    ...
    let mut proof_builder = notarized_session.session().data().build_substrings_proof();  //构造证明

    // Prove the request, while redacting the secrets from it.   证明请求，同时编辑其中的秘密。
    ...
    for header in &request.headers {
        // Only reveal the host header   reveal：泄露
        if header.name.as_str().eq_ignore_ascii_case("Host") {
            proof_builder
                .reveal_sent(header, CommitmentKind::Blake3)  //泄露
                .unwrap();
        } else {
            proof_builder
                .reveal_sent(&header.without_value(), CommitmentKind::Blake3)  //without_value() 不泄露
                .unwrap();
        }
    }

    // Prove the entire response, as we don't need to redact anything   证明整个回应，不需要编辑任何内容
